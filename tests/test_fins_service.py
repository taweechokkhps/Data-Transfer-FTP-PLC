...
class TestFinsService(unittest.TestCase):
    def test_build_fins_read_bit_frame_default_w50_02(self):
        ...
        self.assertEqual(frame[13], 0x00) # Word 50 high byte
        self.assertEqual(frame[14], 0x32) # Word 50 low byte (50 = 0x0032)
        self.assertEqual(frame[15], 0x02) # Bit 2 (0x02)
        self.assertEqual(frame[16], 0x00) # Count high byte
        self.assertEqual(frame[17], 0x01) # Count low byte (1 bit)

    def test_build_fins_read_bit_frame_custom(self):
        frame = build_fins_read_bit_frame(node_id=11, area_code=0x31, word=200, bit=0)
        self.assertEqual(len(frame), 18)
        self.assertEqual(frame[13], 0x00)
        self.assertEqual(frame[14], 0xC8) # Word 200 = 0x00C8
    ...
    @patch('socket.socket')
    def test_check_omron_machine_bit_on(self, mock_socket_cls):
        mock_sock = MagicMock()
        ...
        ok, is_on, msg = check_omron_machine_bit("192.168.0.11", port=9600, timeout=1.0)
        self.assertTrue(ok)
        self.assertTrue(is_on)

    @patch('socket.socket')
    def test_check_omron_machine_bit_off(self, mock_socket_cls):
        mock_sock = MagicMock()
        mock_socket_cls.return_value = mock_sock
    ...
    @patch('socket.socket')
    def test_check_omron_machine_bit_timeout(self, mock_socket_cls):
        mock_sock = MagicMock()
        mock_socket_cls.return_value = mock_sock
